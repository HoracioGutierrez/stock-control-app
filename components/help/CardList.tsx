...
interface DialogState {
  setOpen: (type: string, id: string) => void;
}

function CardList({ cardsData }: CardListProps) {

  const { setOpen } = useDialogStore((state: unknown) => state as DialogState)
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
      {cardsData !== null && cardsData.map((item: CardItem) => (
            ...
            <div className="flex flex-col grow">
              <div className="pt-6 pb-8">
                <div className="flex justify-center items-center bg-accent dark:bg-primary-foreground rounded-full size-16">
                  {config[item.icon as keyof typeof config]}
                </div>
              </div>
