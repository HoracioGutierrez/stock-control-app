...
function UsersDialog({ userId }: Props) {
  ...
  return (
    <CustomDialog title={type ? config[type].title : "Custom Dialog"} fullWidth={type ? config[type].fullWidth : false} >
      {/* {type === "edit-user" && <EditFormContainer entityType="user" userId={userId as string} hasDetails={true} hasVariants={false} />} */}
      {type === "delete-user" && <DeleteUserConfirmationForm userId={userId as string} type={type} />}
    </CustomDialog>
  )
}
